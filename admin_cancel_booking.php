...
<head>
    ...
    <script>
        function cancel_booking(value){
            $.ajax({
                ...
                success: function(response){
                    if(response == 1){
                        window.location = "admin_cancel_booking.php";
                    }
                    else{
        ...
        function change_faculty(){
            ...
            window.location = "admin_cancel_booking.php?faculty=" + faculty;
        }
        ...
        function search_ID(){
            ...
            $.ajax({
                method:"POST",
                url: "process/change_session_bookingCancel.php",
...
<body>
    ...
    <?php 
        ...
        if(isset($_SESSION['id_search_bookingCancel'])){
            $id_search = $_SESSION['id_search_bookingCancel'];
        }
        if(isset($_SESSION['id_user_search_bookingCancel'])){
            $id_user_search = $_SESSION['id_user_search_bookingCancel'];
        }
    ?>
    ...
    <div class="information">
        ...
        <h1 style="color:#007bff;">HỦY ĐẶT KHÁM</h1>
            ...
            <?php
                if($faculty == 'all'){
                    if(isset($id_search) and $id_search!=''){
                        $sql= "SELECT * FROM dat_lich_kham where xac_nhan=TRUE AND id='$id_search'";
                    }
                    else if(isset($id_user_search) and $id_user_search!=''){
                        $sql= "SELECT * FROM dat_lich_kham where xac_nhan=TRUE AND id_tai_khoan='$id_user_search'";
                    }
                    else{
                        $sql= "SELECT * FROM dat_lich_kham where xac_nhan=TRUE ";
                    }
                }
                else{
                    if(isset($id_search) and $id_search!=''){
                        $sql= "SELECT * FROM dat_lich_kham where xac_nhan=TRUE AND id='$id_search' AND khoa='$faculty'";
                    }
                    else if(isset($id_user_search) and $id_user_search!=''){
                        $sql= "SELECT * FROM dat_lich_kham where xac_nhan=TRUE AND id_tai_khoan='$id_user_search' AND khoa='$faculty'";
                    }
                    else{
                        $sql= "SELECT * FROM dat_lich_kham where xac_nhan=TRUE AND khoa='$faculty'";
                    }
                }
                ...
                if (mysqli_num_rows($result) > 0) {
                    while($row = mysqli_fetch_array($result)) {
                        ...
                        $sql1 = "SELECT * FROM thong_tin_tai_khoan_doctor WHERE id_tai_khoan = '$id_doctor'; ";
                        $result1 = mysqli_query($conn, $sql1);
                        if (mysqli_num_rows($result1) > 0){
                            $row = mysqli_fetch_array($result1);
                            $doctor_name = $row['name'];
                            $doctor_surname = $row['sur_name'];
                        }
                        ...
                        echo "<td> $date </td>";
                        echo "<td> $time </td>";
                        echo "<td> $doctor_name $doctor_surname </td>";
                        echo "<td> <button onclick=\"cancel_booking($id_booking)\" style=\"background-color:#007bff\"
                                    class=\"btn btn-primary\"> Hủy </button> </td></tr>";
                    }
                }
            ?>
        </table>
